...
src/
  ...
  models/        Board / Post / AdminUser / Emoticon 도메인 모델
  repositories/  D1 접근 계층 (SQL은 전부 이곳에만 존재)
  media/         이미지 디코딩/WebP 인코딩 (jSquash WASM 코덱, R2 저장은 컨트롤러에서 처리)
...
- **이미지 업로드**: R2에 저장되기 전 서버에서 PNG/JPEG를 디코딩해 WebP로 재인코딩하므로
  원본 파일이 그대로 저장되지 않습니다. HTML/마크다운에서 `<img>`의 src는 업로드 CDN
  오리진(`CDN_ORIGIN`)일 때만 허용되어 임의 오리진 이미지 삽입을 막습니다.
...
```bash
# 1) 원격 D1에 스키마 + 관리자 시드 적용 (migrations/ 안의 파일을 번호 순서대로 실행)
...
- R2 바인딩: `IMAGES` → `inside-storge` (`wrangler.toml`의 `[[r2_buckets]]`와 동일). 이 버킷에
  퍼블릭 커스텀 도메인(`src/config/constants.ts`의 `CDN_ORIGIN`, 현재 `cdn.parin.asia`)이
  연결되어 있어야 업로드된 이미지가 실제로 열람 가능합니다.
- Workers 유료 플랜 필요: WebP 인코딩(WASM)이 무료 플랜의 요청당 10ms CPU 한도를 넘기 쉽습니다
  (`wrangler.toml`의 `[limits] cpu_ms`도 유료 플랜에서만 적용됨).
